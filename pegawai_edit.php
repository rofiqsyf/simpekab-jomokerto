...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            if ($cek->fetch()) {
                $pdo->prepare("UPDATE pegawai SET nip=?,divisi=?,posisi=?,no_telp=?,tgl_masuk=?,status=? WHERE user_id=?")
                    ->execute([$old['nip'],$old['divisi'],$old['posisi'],$old['no_telp']?:null,$old['tgl_masuk'],$old['status'],$id]);
            } else {
                $pdo->prepare("INSERT INTO pegawai (user_id,nip,divisi,posisi,no_telp,tgl_masuk,status) VALUES (?,?,?,?,?,?,?)")
                    ->execute([$id,$old['nip'],$old['divisi'],$old['posisi'],$old['no_telp']?:null,$old['tgl_masuk'],$old['status']]);
            }
